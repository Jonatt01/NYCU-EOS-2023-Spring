...
/******************************************************/
void led_flashing(unsigned int GPIO_PIN);
...
static ssize_t etx_write(struct file *filp, const char __user *buf, size_t len, loff_t *off){
    ...
    int i=0;
    int GPIO_use_array[3] = {GPIO_5, GPIO_6, GPIO_13};
    pr_info("5: %d,6: %d,13: %d\n",rec_buf[0]-'0',rec_buf[1]-'0',rec_buf[2]-'0');
    while(i<3){
        if((rec_buf[i]-'0') > 0){
            pr_info("GPIO_use_array[%d] : %d",i,GPIO_use_array[i]);
            gpio_set_value(GPIO_use_array[i],1);
        }
        else if((rec_buf[i]-'0') < 0){
            pr_info("GPIO_use_array[%d] : %d",i,GPIO_use_array[i]);
            led_flashing(GPIO_use_array[i]);
        }
        else{
            pr_info("GPIO_use_array[%d] : %d",i,GPIO_use_array[i]);
            gpio_set_value(GPIO_use_array[i],0);
        }
        i++;
    }
...
/*
** This function will be called by write function if LED needs to flashing
*/
void led_flashing(unsigned int GPIO_PIN){
